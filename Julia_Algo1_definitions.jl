...
function H_0(num_sites, J)
    ...
    return sum( vcat(   [J/4*sigma_x[i]*sigma_x[i%num_sites+1] for i in 1:num_sites], 
                        [J/4*sigma_y[i]*sigma_y[i%num_sites+1] for i in 1:num_sites], 
                        [J/4*sigma_z[i]*sigma_z[i%num_sites+1] for i in 1:num_sites] ))
end;
...
function U_T(num_sites, A, omega, delta_t, J, times_trapezoid)
    H0 = H_0(num_sites, J)
...
function getFausewehZhuCircuit(num_sites,depth, UT)
    ...
    var_circ_dag = Yao.dispatch!(Yao.chain(num_sites, Yao.put( (([i for i in 1:num_sites]) |> reverse) => Yao.EasyBuild.variational_circuit(Float64, num_sites, depth, pair_ring_new(num_sites), entangler=inverse_cnot))),[-k for k in 1:number_of_parameters |> reverse]);
...
function getOverlapCircuit(num_sites,depth)
    ...
    var_circ_dag = Yao.dispatch!(Yao.chain(num_sites, Yao.put( (([i for i in 1:num_sites]) |> reverse) =>Yao.EasyBuild.variational_circuit(Float64, num_sites, depth, pair_ring_new(num_sites), entangler=inverse_cnot))),[-p for p in 1:number_of_parameters |> reverse]);
